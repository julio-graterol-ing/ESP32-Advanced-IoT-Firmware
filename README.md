...
* **September 21, 2026 | Digital Signal Processing (DSP) & Moving Average ADC Filtering:**
  ...
  **Project: Low-Pass Software Filter for Electro-Magnetic Noise Suppression.**  
  Successfully architected and deployed a mathematical rolling moving average filter algorithm over hardware ADC1 Channel 6 (GPIO 34) to suppress internal radio-frequency noise fluctuations.

  * *The Bottleneck:* The internal analog-to-digital converter (ADC) on the Espressif ESP32 architecture suffers from aggressive electro-magnetic interference (EMI) generated by the Wi-Fi/Bluetooth radio power spikes. Sampling raw data directly results in severe data oscillation (spikes up to ±40 ticks) even under constant voltage inputs, making raw signals completely unusable for high-precision local or cloud automation controls.
  * *The Engineering Fix:* Designed a low-pass DSP abstraction layer (`AdcFilter`) utilizing a localized static ring buffer fixed at 16 sample slots (`FILTER_WINDOW_SIZE`). The algorithm samples the high-impedance hardware channel at maximum clock velocity, structurally subtracts the oldest historical memory slice from an internal fast-sum registry (`runningSum`), injects the newest 12-bit raw conversion, and performs a bitwise division to compute the continuous running mean. This math delivers a stabilized 12-bit integer range (0 to 4095) with zero overhead, isolating background signal fluctuations ahead of inter-task queue injection.
...
```text
├── include/
│   ├── AdcFilter.h        # Rolling moving average digital filter signatures
...
├── src/
│   ├── AdcFilter.cpp      # Low-level ADC1 attenuation and rolling sum buffer execution
